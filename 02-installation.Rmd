...
`nlmixr` can be used on several platforms. The installation can be easy to challenging depending on the platform. We are in the process of making sure the installation work every time. Any help or suggestions are appreciated !
...
To replicate the environment that was used in Windows for `nlmixr` development, you may need administrator rights to write to C:\\, and you should perform the following steps:

1. Install R 3.4.1 (or later) from the R website.
   - For best results, we suggest you use `C:\R\R-3.4.1`, but
     you can also use the default location (`C:\Program Files\R\R-3.4.1`) as well, if really needed. If writing to C:\\ is not allowed, another option could be to install to %AppData%, which is typically in C:\Users\userName\AppData\. Most users have write access to this location. 
   - For 64-bit Windows, it is best practice to include *only* the 64-bit version. 
     ...
     - There could be troubles caused by using "Program Files" because paths with spaces are not always supported on all R packages and typically the "Program Files" directory is read only which has been known to cause problems with some packages being able to run.
2. Install the appropriate version of Rtools for Windows, currently version 3.4, from [here](https://cran.r-project.org/bin/windows/Rtools/).
   ...
   - For best results, use the default location of `C:\Rtools`
     ...
     - If you are on 64-bit windows, please *do not install* the R
       ...
       packages that compile binaries, with unpredictable consequences and it also checks to see if it was installed to a custom location by the `nlmixr` installer.  Similarly, only install 32-bit
...
3. Install a version of Python for Windows.
   ...
   - Regardless of the option you choose, please use 64-bit Python for 64-bit Windows (or 32 bit).
...
5. Install `RxODE`.
   ...
   - As a quick test, you can make sure that R and Python can
     communicate by typing the command `library(SnakeCharmR)`. This will will display the version of python you are using.
  - Another test to make sure that `SymPy` is setup appropriately is to do `library(RxODE)` and `rxSymPyVersion()`. This will display the `SymPy` version.
   - To validate or test the installation of `RxODE` completely, you
     ...
     correctly on your system. (Note that the `testthat` package is required for this, and it will take a long time.) For a short test use `rxTest(FALSE)`.
6. Install `nlmixr`.
...
For the course it will be necessary to have access to your local drive, this can be done by adding an option (-v) in the docker command. In the following example the folder "Users/richard/Document" will be mounted to the docs folder in the home directory:
...
## project management tool (`shinyMixR`)
